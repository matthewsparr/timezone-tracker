...
def getTimezoneList():
    return jsonify(status='success', timezoneList=timezone_list)
...
def addTimezoneForUser():
    if 'userId' not in request.form:
        ...
        if not users.find_one({'_id':user_id}):
            return jsonify(status='error', error="Invalid userId.")
    timezone_name = request.form.get('timezoneName').lower().title()
    if timezone_name not in timezone_list:
        return jsonify(status='error', error="Invalid timezone.")
    else:
...
def getUsers():
    if currentUserPermissions() != "admin" and currentUserPermissions() != "manager":
        return jsonify(status='forbidden')
    else:
